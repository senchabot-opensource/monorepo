...
export const ja: typeof en = {
  ...
  guides: {
    ...
    obs: {
      ...
      transparent: {
        ...
        p1: 'いいえ。チャットボックス、エモートウォール、Sub Sprout、Subathon Timer、配信アラート、サブスク目標、チャット投票、抽選のオーバーレイは、透明な背景に描かれています。クロマキーもフィルタも不要で、OBSの「カスタム CSS」欄もそのままでかまいません。',
      ...
      settings: {
        ...
        subSprout:
          'Sub Sprout: 植物の成長はどこにも保存されません。ページが再読み込みされると、植物は最初の段階に戻ります。',
    ...
    poll: {
      ...
      look: {
        ...
        language:
          '投票の言語: 「結果」やクイック投票の「はい」「いいえ」など、投票に表示される言葉を英語、スペイン語、フランス語、日本語、ポルトガル語、トルコ語から選べます。',
    ...
    alerts: {
      ...
      look: {
        ...
        p2: '各アラートの見出しは24文字まで書き換えられます。空欄にすると「新規サブスク」のような初期設定のままです。ネオンでは英字の見出しが大文字で表示されます。「アラートの言語」はアラートに表示される言葉の言語を決めます。英語、スペイン語、フランス語、日本語、ポルトガル語、トルコ語から選べ、OBSが何語で動いていてもURLに保持されます。',
      },
      min: {
  ...
  faqPage: {
    ...
    freeA:
      'はい。9つのウィジェットとツールはすべて無料です。チャットボックス、エモートウォール、Sub Sprout、Subathon Timer、配信アラート、サブスク目標、チャット投票、抽選、OBS Bridge。有料プランも透かしもプレミアムアカウントもありません。ソースコードはGPL-3.0ライセンスでGitHubに公開しています。',
    ...
    platformsA:
      '11個すべてが両方のプラットフォームに対応しています。チャットボックス、エモートウォール、Sub Sprout、Subathon Timer、配信アラート、サブスク目標、チャット投票、配信カウントダウンは、ひとつのURLでTwitchとKickのチャンネルを同時に聞き取ります。OBS Bridgeは両方のチャットからコマンドを受け取り、許可ユーザーはそれぞれのプラットフォームと一緒に追加します。抽選は一度にひとつのプラットフォーム（TwitchかKick）で動きます。チャットボックスでは、7TVのエモートは両方のプラットフォームで、BTTVとFFZのエモートはTwitchでのみ表示されます。',
  ...
  changelog: {
    ...
    entries: {
      moreLanguages:
        'サイトがスペイン語、フランス語、日本語、ポルトガル語にも対応しました。チャット投票、配信アラート、Subathon Timerのレートも、これらの言語で表示できるようになりました。',
