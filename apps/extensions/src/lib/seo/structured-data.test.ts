...
import { getGuideHead, getSetupPageHead, PAGE_META } from './pages';
import {
  getAppNode,
  getFaqNode,
  getPageGraph,
...
const page = { path: '/setup/raffle', meta: PAGE_META.raffle, image: 'raffle' } as const;
...
describe('page heads', () => {
  ...
  it('give every guide an Article under Home > Guides', () => {
    for (const guide of GUIDES) {
      const graph = graphOf(getGuideHead(guide));
